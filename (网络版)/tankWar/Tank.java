...
public class Tank {
	...
	private IdName idname=new IdName();
...
//				tk1.getImage("H:\\eclipse-standard-kepler-SR2-win32\\MyTank\\image\\tankup.gif")
				...
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankD.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankL.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankLD.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankLU.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankR.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankRD.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankRU.gif")),
				tk1.getImage(Explode.class.getClassLoader().getResource("images/tankU.gif"))
...
//		if(good) 
//		{
			if(TankClient.flag==1 && good)
			{
				bb.draw(g);
				//g.drawString("Tank                id:   " + ID, 5, 120);//????I don't understand the meaning of good?????
				g.drawString("Your          Score:   " + score, 5, 100);
			}
			if(TankClient.flag==2)
			{
				idname.draw(g);
				g.drawString("Tank                id:   " + tc.tankhero.ID, 5, 20);
				g.drawString("Tank           enemies:   " + TankClient.mytanks.size(), 5, 80);
			}
//		}
	...
	private class IdName {
		public void draw(Graphics g) {
			...
			if(ID%2==0)
				...
				g.setColor(Color.RED);
			g.drawString(new String("ID:"+ID+"-"+myname),x, y-10);
//			int w = TANK_WIDTH * life/100 ;
//			g.fillRect(x, y-10, w, 10);/*in the head of my own tank,draw the bloodbar*/
			g.setColor(c);
		}
